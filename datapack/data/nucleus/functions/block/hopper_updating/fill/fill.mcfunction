...
execute unless entity @s[tag=nucleus.prevents_hoppers.allow_top] if block ~ ~1 ~ hopper[facing=down] run data modify block ~ ~1 ~ TransferCooldown set value 2147483646
execute unless entity @s[tag=nucleus.prevents_hoppers.allow_bottom] if block ~ ~-1 ~ hopper[facing=down] run data modify block ~ ~-1 ~ TransferCooldown set value 2147483646
